...
if (!apiKey) {
    ...
    console.log(`[Gemini] v2.9.14 Engine Ready. Key: [${hiddenKey}] (Model: Gemini-2.0-Flash)`);
}
 ...
 * Adjusted for v2.9.14: 5s polling retained.
 */
...
/**
 * 動画や画像を解析する共通関数 (v2.9.14 File API Support)
...
export async function analyzeMedia(mimeType?: string, dataBase64?: string, prompt: string = "", filePath?: string) {
    ...
    if (mimeType?.startsWith('video/') && filePath) {
        console.log(`[Gemini] v2.9.14 (2.0-Flash FileAPI) Video Analysis Start...`);
        ...
        try {
            ...
            // Verified model name: gemini-2.0-flash (Available for this key)
    ...
    console.log(`[Gemini] v2.9.14 (2.0-Flash Inline) Analysis Start...`);
